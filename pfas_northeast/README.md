...
- `code/modeling/backward_elim_pfas.py` — backward feature elimination for the Northeast arm (PFAS only — the shared `midwest8`/`national`/lithium code paths in the original script were removed here, along with the now-broken path they relied on, since this folder never had those source files to begin with).
