"""
Add airport distance features to national_facility_features_v7.csv → v8.csv.
...
New columns:
  dist_airport_km         — km to nearest FAA Part 139 certified airport
  count_airports_in_huc12 — airports within the source HUC-12 watershed
"""
...
AIRPORT_FILE  = SRC  / 'arp-aas-part139-cert-status-table-2024-09-05.xlsx'
IN_CSV        = BASE / 'national_facility_features_v7.csv'
OUT_CSV       = BASE / 'national_facility_features_v8.csv'
...
def load_airport_coords():
    df = pd.read_excel(AIRPORT_FILE, header=5)
    df.columns = df.iloc[0]
    df = df.iloc[1:].reset_index(drop=True)
    df = df[df['State'].notna()].copy()
    loc_ids = df['Airport Identifier (LOC ID)'].dropna().str.strip().tolist()
    iata = airportsdata.load('IATA')
    icao = airportsdata.load('ICAO')
    coords = []
    for code in loc_ids:
        if code in iata:
            a = iata[code]; coords.append([a['lat'], a['lon']])
        elif ('K' + code) in icao:
            a = icao['K' + code]; coords.append([a['lat'], a['lon']])
    arr = np.array(coords)
    print(f"  Airport coordinates: {len(arr):,} / {len(loc_ids):,} matched")
    return arr


def count_airports_per_huc12(airport_coords, geojson_path, checkpoint_path=None):
    counts = {}
    if checkpoint_path and Path(checkpoint_path).exists():
        with open(checkpoint_path) as f:
            counts = json.load(f)
        print(f"  Loaded {len(counts):,} cached airport counts from checkpoint")
    ...
    airport_points = [Point(lon, lat) for lat, lon in airport_coords]
    total = len(features)
    new_count = 0

    for huc12, poly in polys.items():
        minx, miny, maxx, maxy = poly.bounds
        n = sum(1 for p in airport_points
    ...
    print(f"  HUC-12s with ≥1 airport: {sum(v > 0 for v in counts.values()):,} / {len(counts):,}")
    return counts
...
def main():
    ...
    print("\nComputing source-water-corrected airport distances...")
    dists = haversine_min_dist(eff_latlon, airport_coords)
    if mix_indices and len(airport_coords) > 0:
        dists_seller = haversine_min_dist(mix_latlon, airport_coords)
    ...
    df['dist_airport_km'] = np.where(np.isfinite(dists), np.round(dists, 4), np.nan)
    print(f"  min={np.nanmin(dists):.1f} km  median={np.nanmedian(dists):.1f} km  "
          f"max={np.nanmax(dists):.1f} km")

    # ── Count airports per HUC-12 (all polygons in geojson) ──────────────────
    print("\nCounting airports per HUC-12 (national polygons)...")
    AIRPORT_CKPT = BASE / 'airport_huc12_counts_checkpoint.json'
    huc12_counts = count_airports_per_huc12(airport_coords, HUC12_GEOJSON, AIRPORT_CKPT)
    ...
    for i in range(len(df)):
        ...
        vals.append(count)
    df['count_airports_in_huc12'] = vals
    print(f"  Facilities with ≥1 airport in HUC-12: {(df['count_airports_in_huc12'] > 0).sum():,}")

    print(f"\nSaving to {OUT_CSV}...")
    df.to_csv(OUT_CSV, index=False)
    print(f"Done. New columns: dist_airport_km, count_airports_in_huc12")
